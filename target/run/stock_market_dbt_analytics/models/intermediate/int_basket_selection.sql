  ...
  create or replace view `dbt-demo-project-485817`.`dbt_dev`.`int_basket_selection`
  OPTIONS()
  as -- Score every asset at each week-end and select the basket.
...
with rolling_stats as (
    select * from `dbt-demo-project-485817`.`dbt_dev`.`int_rolling_stats`
),
